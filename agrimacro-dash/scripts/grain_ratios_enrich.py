...
try:
    if BCB.exists():
        ...
        if isinstance(bcb, dict):
            ...
            for k in ["usd_brl","brl_usd","ptax","cambio","exchange_rate","BRL","USD_BRL"]:
                v = bcb.get(k)
                if v:
                    try:
            ...
            # Tentar qualquer float razoavel
            if brl_usd == 5.8:
    ...
    log(f"BRL/USD: {brl_usd:.2f}")
except Exception as e:
    log(f"BRL/USD fallback 5.8 ({e})")
...
try:
    if not COT.exists():
        ...
        for sym, grain in [("ZC","corn"), ("ZS","soy"), ("ZW","wheat")]:
            ...
            log(f"  {sym} ({rtype:<15}) mm={lmm:+.0f}  idx={lidx:.0f}  n={len(s)}")
...
bdi_val = (gr.get("arbitrage", {}).get("bdi", {}).get("value") or 1450)
freight_gulf   = round(bdi_val * 0.013 + 2.8, 1)
freight_santos = round(bdi_val * 0.017 + 3.5, 1)
log(f"BDI={bdi_val}  Frete Gulf={freight_gulf}  Santos={freight_santos}")
...
# FOBs
fob_br_soy  = fob_data["paranagua"]["soy"]  or (round(soy_cme  * 0.97, 1) if soy_cme  else None)
fob_br_corn = fob_data["paranagua"]["corn"] or (round(corn_cme * 0.96, 1) if corn_cme else None)
fob_ar_soy  = fob_data["rosario"]["soy"]    or (round(soy_cme  * 0.95, 1) if soy_cme  else None)
fob_ar_corn = fob_data["rosario"]["corn"]   or (round(corn_cme * 0.94, 1) if corn_cme else None)
...
cif = {
    ...
    "corn_arg": safe_add(fob_ar_corn,    freight_santos * 0.92),
    "soy_us":   safe_add(fob_gulf_soy,   freight_gulf),
    "soy_br":   safe_add(fob_br_soy,     freight_santos),
    "soy_arg":  safe_add(fob_ar_soy,     freight_santos * 0.92),
...
def build_scorecard(grain, snap, current_cot, cif, sp):
    ...
    cot_idx = float(cot_d.get("cot_index") or 50)
